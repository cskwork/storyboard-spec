...
산출물 한 줄 정의: 화면 1개 = 페이지 1개. **왼쪽 = 화면 + 번호 콜아웃**, **오른쪽 = 요소별 설명표(동작·데이터·예외)**. 그리고 썸네일 카드 **보드(index)** 가 각 페이지로 링크.
...
### 모드 A — 실제 UI 재사용 (문서화 · 픽셀 일치)
대상 앱의 **진짜 마크업/클래스를 그대로** `.sb-screen` 안에 붙이고, **대상 앱의 진짜 CSS를 상대경로로 링크**한다. 그러면 왼쪽이 실제 화면과 똑같이 렌더된다. (Figma 소스면 §8 의 `figma-cli` 경로로 export 이미지/스펙을 먼저 확보.)
