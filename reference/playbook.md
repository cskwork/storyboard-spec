...
산출물 한 줄 정의: 화면 1개 = 페이지 1개. **왼쪽 = 실제 화면(픽셀까지 재현) + 번호 콜아웃**, **오른쪽 = 요소별 설명표(동작·데이터·예외)**. 그리고 썸네일 카드 **보드(index)** 가 각 페이지로 링크.
...
- **재현 소스**: 실제 UI가 있는가? (있으면 그 CSS 재사용 = A안, 없으면 와이어프레임 = B안)
...
코드/디자인을 읽고 화면의 **상호작용 요소**를 빠짐없이 뽑는다. 요소 하나당:
...
### A안 — 실제 UI 재사용 (권장, 픽셀 일치)
대상 앱의 **진짜 마크업/클래스를 그대로** `.sb-screen` 안에 붙이고, **대상 앱의 진짜 CSS를 상대경로로 링크**한다. 그러면 왼쪽이 실제 화면과 똑같이 렌더된다.
...
### B안 — 와이어프레임 (UI 없을 때)
storyboard.css 의 sb- 클래스로 간단한 박스/버튼/필드를 직접 만들어 화면을 묘사한다. 충실도는 낮아도 "어떤 요소가 어디 있는지"는 전달.
...
- **모바일/네이티브**: A안이 어려우면 스크린샷을 `.sb-screen` 배경/`<img>`로 깔고 그 위에 절대좌표로 `.sb-cue`를 얹는다(이땐 cue 부모를 `position:relative`로). 오른쪽 표는 동일.
- **프레임워크 앱(React/Vue 등)**: 컴포넌트를 정적 HTML로 한 번 렌더(또는 빌드 산출물 HTML)해 재현하고 그 CSS를 링크.
- **Figma/목업만 있을 때**: B안 와이어프레임 또는 export 이미지 + 절대좌표 cue.
