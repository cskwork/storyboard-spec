...
4. **Build pages** — from `storyboard-page.html`: RIGHT = `sb-notes` table as **real HTML text** (never a baked image), one row per cue. LEFT = the screen as the picture + `sb-mark`/`sb-cue` callouts — **mode A**: a Figma/screenshot `<img>` (canonical), or a live real-markup+CSS render; **mode B**: a wireframe from `sb-` boxes/fields/buttons.
